...
class feetechsts(protocol_packet_handler):
    ...
    def set_id(self, motor_id, new_id):
        ...
        if current_id != new_id:
            if self.verbose: print(f"[feetechsts::set_id] (EEPROM) Setting ID to: {new_id}...")
            ...
            if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
                if self.verbose: print(f"[feetechsts::set_id] (EEPROM) ID correctly set to: {new_id}")
    
        else:
            if self.verbose: print(f"[feetechsts::set_id] ID was already: {new_id}. Not setting")
            return current_id
    ...
    def set_mode(self, motor_id, mode):
        ...
        if current_mode != mode:
            if self.verbose: print(f"[feetechsts::set_mode] (EEPROM) Setting mode to: {mode}...")
            ...
            # result
            if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
                if self.verbose: print(f"[feetechsts::set_mode] (EEPROM) mode correctly set to: {mode}")
    
        else:
            if self.verbose: print(f"[feetechsts::set_mode] Mode was already: {mode}. Not setting")
            return current_mode

    #
    def set_lock(self, motor_id, lock_status):
        current_lock = self.get_lock(motor_id)
        
        # write on EEPROM only if needed
        if current_lock != lock_status:
            if self.verbose: print(f"[feetechsts::set_lock] (EEPROM) Setting lock status to: {lock_status}...")
            ...
            # result
            if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
                if self.verbose: print(f"[feetechsts::set_lock] (EEPROM) lock status correctly set to: {lock_status}")
    
        else:
            if self.verbose: print(f"[feetechsts::set_lock] lock status was already: {lock_status}. Not setting")
            return current_lock

    #
    def set_angle_min(self, motor_id, angle):
        min_angle = self.get_angle_min(motor_id)
        
        # write on EEPROM only if needed
        if min_angle != angle:
            if self.verbose: print(f"[feetechsts::set_angle_min] (EEPROM) Setting min angle to: {angle}...")
            ...
            # result
            if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
                if self.verbose: print(f"[feetechsts::set_angle_min] (EEPROM) min angle correctly set to: {angle}")
    
        else:
            if self.verbose: print(f"[feetechsts::set_angle_min] min angle was already: {angle}. Not setting")
            return min_angle

    #
    def set_angle_max(self, motor_id, angle):
        max_angle = self.get_angle_max(motor_id)
        
        # write on EEPROM only if needed
        if max_angle != angle:
            if self.verbose: print(f"[feetechsts::set_angle_max] (EEPROM) Setting max angle to: {angle}...")
            ...
            # result
            if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
                if self.verbose: print(f"[feetechsts::set_angle_max] (EEPROM) max angle correctly set to: {angle}")
    
        else:
            if self.verbose: print(f"[feetechsts::set_angle_max] max angle was already: {angle}. Not setting")
            return max_angle

    #
    def set_torque_max(self, motor_id, torque):
        max_torque = self.get_torque_max(motor_id)
        
        # write on EEPROM only if needed
        if torque != max_torque:
            if self.verbose: print(f"[feetechsts::set_torque_max] (EEPROM) Setting max torque to: {torque}...")
            ...
            # result
            if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
                if self.verbose: print(f"[feetechsts::set_torque_max] (EEPROM) max torque correctly set to: {torque}")
    
        else:
            if self.verbose: print(f"[feetechsts::set_torque_max] max torque was already: {torque}. Not setting")
            return max_torque

    #
    def set_force_startup_min(self, motor_id, force):
        present_force = self.get_force_startup_min(motor_id)
        
        # write on EEPROM only if needed
        if force != present_force:
            if self.verbose: print(f"[feetechsts::set_force_startup_min] (EEPROM) Setting min startup force to: {force}...")
            ...
            # result
            if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
                if self.verbose: print(f"[feetechsts::set_force_startup_min] (EEPROM) min startup force correctly set to: {force}")
    
        else:
            if self.verbose: print(f"[feetechsts::set_force_startup_min] min startup force was already: {force}. Not setting")
    ...
    #
    def set_torque(self, motor_id, torque_status):
        if self.verbose: print(f"[feetechsts::set_torque] Setting torque to: {torque_status}...")
        ...
        # result
        if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
            if self.verbose: print(f"[feetechsts::set_torque] torque correctly set to: {torque_status}")

    #
    def set_acceleration(self, motor_id, acceleration):
        if self.verbose: print(f"[feetechsts::set_acceleration] Setting acceleration to: {acceleration}...")
        ...
        # result
        if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
            if self.verbose: print(f"[feetechsts::set_acceleration] acceleration correctly set to: {acceleration}")

    #
    def set_position(self, motor_id, position):
        if self.verbose: print(f"[feetechsts::set_position] Setting position to: {position}...")
        ...
        # result
        if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
            if self.verbose: print(f"[feetechsts::set_position] position correctly set to: {position}")

    #
    def set_speed(self, motor_id, speed):
        if self.verbose: print(f"[feetechsts::set_speed] Setting speed to: {speed}...")
        ...
        # result
        if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
            if self.verbose: print(f"[feetechsts::set_speed] speed correctly set to: {speed}")

    #
    def set_torque_limit(self, motor_id, torque):
        if self.verbose: print(f"[feetechsts::set_torque_limit] Setting torque limit to: {torque}...")
        ...
        # result
        if (sts_comm_result == COMM_SUCCESS) & (sts_error == 0):
            if self.verbose: print(f"[feetechsts::set_torque_limit] torque limit correctly set to: {torque}")
